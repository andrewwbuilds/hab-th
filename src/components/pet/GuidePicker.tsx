...
export function GuidePicker({ pet, returnTo = null }: { pet: PetSpec | null; returnTo?: string | null }) {
  ...
  return <section className="guide-picker">
    ...
    <div className="guide-options" aria-label="Choose your guide">
      ...
      <button disabled={pending} type="button" aria-pressed={kind === "photo"} onClick={() => select("photo")}><span className="guide-option-icon">▧</span><strong>Use your photo</strong><span>You, with a little spice added</span></button>
    </div>
    <fieldset disabled={pending} className="guide-editor">
      ...
      {kind === "photo" && <div className="flex flex-col items-start gap-4">
        ...
        {portrait && <><PixelPortrait src={portrait} kind="portrait" name="Your anime portrait preview" size={192} /><div className="flex items-center gap-3"><p>Your photo, with a little spice added.</p>{photo && <Button onClick={() => void stylize(photo)}>Redraw</Button>}</div></>}
        {portraitStatus === "drawing" && <div className="flex items-center gap-3" aria-live="polite"><PixelPortrait src={photo} name="Your photo" size={192} resolution={128} className="opacity-50" /><p>Adding a little spice to your photo… this takes about ten seconds.</p></div>}
